...
namespace PortfolioProjectNight.Controllers
{
    public class StatisticsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.maximumRateSkillName = context.Skill.Where(x => x.Rate == maxRate).Select(y => y.SkillName).FirstOrDefault();

            ViewBag.getMessageCountBySubjectReferances = context.Contact.Where(x=>x.Subject == "Referans").Count();  
            ViewBag.getMesssageCountByEmailContainHAndIsReadTrue = context.Contact.Where(x=>x.IsRead == true && x.Email.Contains("h")).Count();
